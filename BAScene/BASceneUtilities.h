...
static inline BAUInt BAULPDiffFloats(CGFloat f1, CGFloat f2) {
	BAUInt r;
	ULPDiffs(r, f1, f2);
	return r;
}

static inline BOOL BAEqualFloatsULP(CGFloat f1, CGFloat f2) {
	BAUInt r;
	ULPDiffs(r, f1, f2);
    return r <= ULPS_DELTA;
}
...
extern void BARandomSphereSurfaceLocation(CGFloat cx, CGFloat cy, CGFloat cz, CGFloat r, CGFloat *x, CGFloat *y, CGFloat *z);
...
#pragma mark -
#pragma mark Ranges
static inline BARangef BAMakeRangef(CGFloat p, CGFloat d) {
	BARangef r;
	r.p = p;
	r.d = d;
	return r;
...
static inline BAPointf BAMakePointf(CGFloat x, CGFloat y, CGFloat z) {	
	BAPointf point;
	point.x = x;
	point.y = y;
	point.z = z;
...
static inline BAPointf BATranslatePointf(BAPointf p, CGFloat dx, CGFloat dy, CGFloat dz) {
	BAPointf p1;
	p1.x = p.x+dx;
	p1.y = p.y+dy;
	p1.z = p.z+dz;
...
static inline CGFloat BADotProductVectors3(BAVector3 a, BAVector3 b) {
...
static inline BAPoint4f BALineSolutionX(BALine l, CGFloat x) {
    
    BAPoint4f r = BAMakePoint4f(x, 0, 0, 1.0f);
    
    if(l.v.x != 0) {
        CGFloat ratio = (l.v.x-l.p.x)/(x-l.v.x);
...
static inline BAPoint4f BALineSolutionY(BALine l, CGFloat y) {
    
    BAPoint4f r = BAMakePoint4f(0, y, 0, 1.0f);
    
    if(l.v.y != 0) {
        
        CGFloat ratio = (l.v.y-l.p.y)/(y-l.v.y);
...
static inline BAPoint4f BALineSolutionZ(BALine l, CGFloat z) {

    BAPoint4f r = BAMakePoint4f(0, 0, z, 1.0f);
    
    if(l.v.z != 0) {
        CGFloat ratio = (l.v.z-l.p.z)/(z-l.v.z);
        r.x = (l.v.x - l.p.x)*ratio + l.v.x;
        r.y = (l.v.y - l.p.y)*ratio + l.v.y;
    }
    else if(l.p.z != z) {
...
static inline BAMatrix3x3f BAMakeColMatrix3x3f(CGFloat a00, CGFloat a01, CGFloat a02,
											   CGFloat a10, CGFloat a11, CGFloat a12,
											   CGFloat a20, CGFloat a21, CGFloat a22) {
	BAMatrix3x3f m;
...
static inline BAMatrix3x3f BAMakeRowMatrix3x3f(CGFloat a00, CGFloat a01, CGFloat a02,
											   CGFloat a10, CGFloat a11, CGFloat a12,
											   CGFloat a20, CGFloat a21, CGFloat a22) {
	BAMatrix3x3f m;
...
static inline BAMatrix4x4f BAMakeColMatrix4x4f(CGFloat a00, CGFloat a01, CGFloat a02, CGFloat a03,
											   CGFloat a10, CGFloat a11, CGFloat a12, CGFloat a13,
											   CGFloat a20, CGFloat a21, CGFloat a22, CGFloat a23,
											   CGFloat a30, CGFloat a31, CGFloat a32, CGFloat a33) {
	BAMatrix4x4f m;
...
static inline BAMatrix4x4f BAMakeRowMatrix4x4f(CGFloat a00, CGFloat a01, CGFloat a02, CGFloat a03,
											   CGFloat a10, CGFloat a11, CGFloat a12, CGFloat a13,
											   CGFloat a20, CGFloat a21, CGFloat a22, CGFloat a23,
											   CGFloat a30, CGFloat a31, CGFloat a32, CGFloat a33) {
	BAMatrix4x4f m;
...
extern NSUInteger BAMatrixSmallestRowOrColumn( BOOL *isRow, CGFloat *m, NSUInteger n);
extern CGFloat BAMatrixMinor( CGFloat *m, NSUInteger row, NSUInteger column, NSUInteger n );
extern CGFloat BAMatrixDeterminant(CGFloat *m, NSUInteger n);


static inline CGFloat BAMatrixDeterminant3x3f(BAMatrix3x3f m) {
    return BAMatrixDeterminant(m.i, 3);
}

static inline CGFloat BAMatrixDeterminant4x4f(BAMatrix4x4f m) {
...
#define BAVectorTransform( _v_, _a_, _m_, _c_ ) do {\
	CGFloat *_pv_ = (CGFloat *)&_v_;\
	CGFloat *_pa_ = (CGFloat *)&_a_;\
	for(unsigned _i_=0; _i_<_c_; ++_i_)\
		for(unsigned _j_=0; _j_<_c_; ++_j_)\
...
static inline BAMatrix3x3f BARotationMatrix3x3f(CGFloat angle) {

	CGFloat cosa = cosine(angle), sina = sine(angle);
...
static inline BAMatrix3x3f BAArbitraryRotationMatrix3x3f(BAPointh p, CGFloat angle) {
	
	if(BAIsZeroPointh(p))
		return BARotationMatrix3x3f(angle);
	
	CGFloat cosa = cosine(angle), sina = sine(angle), x = p.x, y = p.y;
...
static inline BAMatrix4x4f BAXAxisRotationMatrix4x4f(CGFloat angle) {
	CGFloat cosa = cosine(angle), sina = sine(angle);
...
static inline BAMatrix4x4f BAYAxisRotationMatrix4x4f(CGFloat angle) {
	CGFloat cosa = cosine(angle), sina = sine(angle);
...
static inline BAMatrix4x4f BAZAxisRotationMatrix4x4f(CGFloat angle) {
	CGFloat cosa = cosine(angle), sina = sine(angle);
...
static inline BAMatrix4x4f BAYZRotationMatrix4x4f(BAPoint4f p, CGFloat angle) {
	
	if(BAPointIsOrigin(p)) return BAXAxisRotationMatrix4x4f(angle);
	
	CGFloat b = p.y, c = p.z;
	CGFloat cosa = cosine(angle), ccosa = 1-cosa, sina = sine(angle);
...
static inline BAMatrix4x4f BAXZRotationMatrix4x4f(BAPoint4f p, CGFloat angle) {
...
static inline BAMatrix4x4f BAXYRotationMatrix4x4f(BAPoint4f p, CGFloat angle) {
...
static inline BAMatrix4x4f BAOriginRotationMatrix4x4f(BAVector3 vector, CGFloat angle) {

	BAVector3 nv = BANormalizeVector3(vector);

	CGFloat u = nv.x, v = nv.y, w = nv.z;
	CGFloat uu = u*u, vv = v*v, ww = w*w, uv = u*v, uw = u*w, vw = v*w;
	CGFloat cosa = cosine(angle), ccosa = 1-cosa, sina = sine(angle);
	CGFloat usina = u*sina, vsina = v*sina, wsina = w*sina, uvccosa = uv*ccosa, uwccosa = uw*ccosa, vwccosa = vw*ccosa;
...
static inline BAMatrix4x4f BAArbitraryRotationMatrix4x4f(BALine axis, CGFloat angle) {
	...
	CGFloat a = axis.p.x, b = axis.p.y, c = axis.p.z, u = nv.x, v = nv.y, w = nv.z;
	CGFloat uu = u*u, vv = v*v, ww = w*w, uv = u*v, uw = u*w, vw = v*w;
	CGFloat uu_vv = uu + vv, uu_ww = uu + ww, vv_ww = vv + ww;
	CGFloat cosa = cosine(angle), ccosa = 1-cosa, sina = sine(angle);
	CGFloat usina = u*sina, vsina = v*sina, wsina = w*sina, uvccosa = uv*ccosa, uwccosa = uw*ccosa, vwccosa = vw*ccosa;
	CGFloat au = a*u, av = a*v, aw = a*w, bu = b*u, bv = b*v, bw = b*w, cu = c*u, cv = c*v, cw = c*w;
...
static inline BAMatrix3x3f BATranslateMatrix3x3f(BAMatrix3x3f m, BAVector v) {
	return BAMultiplyMatrix3x3f(BATranslationMatrix3x3f(v), m);
}

static inline BAMatrix4x4f BATranslateMatrix4x4f(BAMatrix4x4f m, BAVector3 v) {
	return BAMultiplyMatrix4x4f(BATranslationMatrix4x4f(v), m);
}
...
static inline BALocationf BAMakeLocationf(CGFloat x, CGFloat y, CGFloat z, CGFloat w) {	
	BALocationf loc;
	loc.p.x = x;
	loc.p.y = y;
	loc.p.z = z;
...
// Orientations
static inline BAOrientationf BAMakeOrientationf(CGFloat x, CGFloat y, CGFloat z) {	
	BAOrientationf o;
	o.r.x = x;
	o.r.y = y;
	o.r.z = z;
...
static inline BASizef BAMakeSizef(CGFloat w, CGFloat h, CGFloat d) {
	BASizef size;
	size.w = w;
	size.h = h;
	size.d = d;
...
static inline BARegionf BAMakeRegionf(CGFloat x, CGFloat y, CGFloat z, CGFloat w, CGFloat h, CGFloat d) {
	BARegionf region;
...
static inline CGFloat BAMinXf(BARegionf region) {
	return region.origin.p.x;
}

static inline CGFloat BAMinYf(BARegionf region) {
	return region.origin.p.y;
}

static inline CGFloat BAMinZf(BARegionf region) {
	return region.origin.p.z;
}

static inline CGFloat BAMaxXf(BARegionf region) {
	return region.origin.p.x + region.volume.s.w;
}

static inline CGFloat BAMaxYf(BARegionf region) {
	return region.origin.p.y + region.volume.s.h;
}

static inline CGFloat BAMaxZf(BARegionf region) {
	return region.origin.p.z + region.volume.s.d;
}
...
static inline BARegionf BAIntersectionRegionf(BARegionf a, BARegionf b) {
	...
	for(NSUInteger i=0;i<3;++i) {
		...
		CGFloat min = a.origin.i[i] > b.origin.i[i] ? a.origin.i[i] : b.origin.i[i];
		CGFloat max = maxA[i] < maxB[i] ? maxA[i] : maxB[i], length = max - min;
...
static inline BARegionf BAInsetRegionf(BARegionf region, CGFloat offset) {
	BARegionf result;
	result.origin.p.x = region.origin.p.x + offset;
	result.origin.p.y = region.origin.p.y + offset;
	result.origin.p.z = region.origin.p.z + offset;
