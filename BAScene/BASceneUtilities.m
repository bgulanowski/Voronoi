...
NSUInteger BAMatrixSmallestRowOrColumn( BOOL *isRow, CGFloat *m, NSUInteger n) {
...
CGFloat BAMatrixMinor( CGFloat *m, NSUInteger row, NSUInteger column, NSUInteger n ) {
    
    assert(n > 2);

    CGFloat *minorMatrix = malloc(sizeof(CGFloat)*(n-1)*(n-1));
    ...
    CGFloat det = BAMatrixDeterminant(minorMatrix, n-1);
...
CGFloat BAMatrixDeterminant(CGFloat *m, NSUInteger n) {
        ...
        if(isRow) {
            for(NSUInteger i=0; i<n; ++i) {
                ...
                if(!BAEqualFloats(0, m[index])) {
                    ...
                    CGFloat minor = BAMatrixMinor(m, rc, i, n);
            ...
            for(NSInteger i=0; i<n; ++i) {
                ...
                if(!BAEqualFloats(0, m[index])) {
                    NSInteger icf = -(i&1)*2+1;
                    CGFloat minor = BAMatrixMinor(m, i, rc, n);
                    result += icf * rccf * m[index] * minor;
                }
            }
        }
...
static CGFloat *BAMatrixInverse(CGFloat *m, NSUInteger n) {
    
    CGFloat *r = malloc(sizeof(CGFloat)*n*n);
    
    CGFloat dinv = 1.0f/BAMatrixDeterminant(m, n);
...
BAMatrix3x3f BAMatrixInverse3x3f(BAMatrix3x3f m) {
    ...
    BAMatrix3x3f result;
    CGFloat *temp = BAMatrixInverse(m.i, 3);
...
BAMatrix4x4f BAMatrixInverse4x4f(BAMatrix4x4f m) {
    ...
    BAMatrix4x4f result;
    CGFloat *temp = BAMatrixInverse(m.i, 4);
...
BARegionf BAUnionRegionf(BARegionf a, BARegionf b) {
	BAUnionRegionT( BARegionf, CGFloat, f );
}
...
void BARandomSphereSurfaceLocation(CGFloat cx, CGFloat cy, CGFloat cz, CGFloat r, CGFloat *x, CGFloat *y, CGFloat *z) {
	...
	CGFloat r_2 = r*r;
	CGFloat x_2 = BARandomCGFloatInRange(0, r_2);
	CGFloat y_2 = BARandomCGFloatInRange(0, r_2-x_2);
	CGFloat z_2 = r_2-(x_2+y_2);
	
	*x = sqrt(x_2) * BARandomSignedness();
	*y = sqrt(y_2) * BARandomSignedness();
	*z = sqrt(z_2) * BARandomSignedness();
}
...
+ (void)initialize {
	if([self class] == [BASceneUtilities class]) {
		// randomness tests
		NSLog(@"size of float: %u; size of CGFloat: %u; size of GLfloat: %u", sizeof(float), sizeof(CGFloat), sizeof(GLfloat) );
		...
		NSLog(@"10 x BARandomCGFloat():");
		for(int i=0;i<10;++i)
			NSLog(@"  %f", BARandomCGFloat());
